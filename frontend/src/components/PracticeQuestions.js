...
function Assessment() {
  ...
  const [questions, setQuestions] = useState([]);  // Now stores questions and their explanations
  ...
  const subjectTopics = {
    ...
    Science: ['Mechanics', 'Thermodynamics','Optics', 'Electromagnetism', 'Waves and Oscillations', 'Modern Physics','Fluid Mechanics','Properties of Matter','Nuclear Physics','Acoustics','Electronics', 
      'Polymer Chemistry', 
      'Biology', 
        ...
        <div className="assessment-form">
          ...
          {questions.length > 0 && currentQuestion && !finished && (
            <div className="question-card">
              ...
              {/* Explanation section UI */}
              {showAnswer && currentQuestion.explanation && (
                <div className="explanation-section scrollable">
                  <h4>Explanation:</h4>
                  <ul>
                    {currentQuestion.explanation.split('•').map((point, idx) => (
          ...
          {finished && (
            <p className="completion-message"> You have completed your Practice.</p>
          )}
        </div>
      </div>
    </div>
